...
- _Домен:_ `claude-code`, `инфра`, `инструменты`, `безопасность`, `процессы`
...
1. Создать `*.md` / `*.mdx` в нужной папке раздела. Frontmatter — по конвенции выше (как минимум `title`, `tags`, `draft: true`).
2. Раздел с `autogenerate` подхватит файл сам. Порядок — через `sidebar.order` во frontmatter.
3. Вписать строку в `REVIEW.md`.
