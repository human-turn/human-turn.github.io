...
| `pi/`                    | Pi Coding Agent                 | вводная, конфигурация, пакеты, экстеншены                |
...
- _Домен:_ `claude-code`, `pi`, `инфра`, `инструменты`, `безопасность`, `процессы`, `росреалии`, `библиотека` (служебные блоки/эталоны)
